...
namespace KkomaKnight.Game
{
    ...
    public sealed class SeasonPassScreen : GameScreen
    {
        ...
        /// <para>
        /// <b>어떻게 쟀나</b>: <b>레퍼런스에서는</b> 그라데이션이 <b>세로로만</b> 변하므로 <b>같은 y 에서 가로로</b> 훑으면 남는 흔들림이 곧 무늬다
        /// (`19_pass.jpg` · 열의 칸 없는 왼쪽 여백 · y 34·38·42%). 세 열 모두 <b>밝기 폭 ≈ 9/255</b> 였고 바탕이 ≈ 90 이라
        /// 흰 무늬가 그만큼 들어 올리려면 알파 ≈ 9 ÷ (255 − 90) ≈ 0.05 다.
        /// </para>
        /// <para>
        /// ⚠ <b>T344 뒤로는 «우리 그림» 에서 축이 뒤집혔다 — 이 방법을 그대로 쓰면 안 된다</b>(2026-09-09 · 결정 971).
        /// 이 방법이 서는 자리는 «<b>재려는 것이 변하지 않는 축</b> 으로 훑는다» 하나뿐인데, 주인 지시로 <b>우리</b> 열은 이제
        /// <b>가로</b>로 변한다(<see cref="UiKit.GradientCardSide"/>). ⇒ <b>레퍼런스(`19_pass.jpg`)를 잴 때만 «고정 y · 가로 훑기»</b> 이고,
        /// <b>찍힌 `screens` 19 를 잴 때는 «고정 x · <u>세로</u> 훑기»</b> 다. 축을 안 바꾸면 그라데이션 변화를 무늬로 세어
        /// 알파를 또 부풀린다(위 ⚑ 이 이미 한 번 치른 값이다).
        /// </para>
        /// <para>

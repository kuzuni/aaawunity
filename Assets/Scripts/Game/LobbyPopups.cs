...
namespace KkomaKnight.Game
{
    ...
    public static class LobbyPopups
    {
        ...
        static string RecipeIcon(string part)
        {
            ...
            return App.Assets != null && App.Assets.Has(key) ? key : Recipes.IconKey;
        }
        ...
        public static RectTransform Cell(Transform parent, Layout.R parentR, Layout.R cellR, string frameColor, string iconKey, string qty = null, bool locked = false, string name = "Cell", bool qtyBand = false)
        {
            ...
            if (!string.IsNullOrEmpty(qty))
            {
                ...
                // ⛳ T459(주인 2026-09-12 «퀘스트 팝업·출석 팝업·데일리 기프트 **다 그렇게 해 줘** · 출석 팝업 현재 다르게 나옴») —
                //   두 갈래가 남긴 차이는 **글자 크기를 어디서 뽑는가** 하나뿐이었다: 위는 칸 높이에서, 아래는 보조 크기 고정.
                //   그래서 같은 칸 크기에서도 출석만 숫자가 크게 났고, 그것이 주인이 본 «다르게 나옴» 이다.
                //   ⚠ 종전 주석은 «칸 px 를 모르는 부르는 자리가 있다» 고 적었는데 **이 함수에서는 참이 아니다** —
                //     `cellR` 은 필수 인자이고 `Within(parentR)` 로 나누기 «전» 의 값이라 그대로 프레임 % 다(Core/Layout.R.Within).
                //     곧 모든 부르는 자리가 이미 그 수를 넘기고 있었다. 갈래를 지우고 한 문법으로 모은다.
                else GearUi.CellQty(cell, qty, cellR.H);
            }
        ...
        //     ⚑ **아래 여백은 기사와 적이 다르다**(기사 22.9% ↔ 적 13.4% · `screens` 30 실측 · 리그의 키가 달라서다).
        //       첫 회차에 «같은 리그니 같겠지» 로 기사 값을 적에게도 쓴 탓에 적이 4.8%p 낮게 섰다 —
        //       사진이 그것을 잡았고 자는 못 잡았다(자가 그 «같겠지» 를 그대로 믿고 있었다 · 결정 970).
        //     적: h 32.7 → 46.3(기사와 같은 배율 ×1.4167) · y = 83.5 + 0.134 × 46.3 − 46.3 = **43.4**.
    ...
    public sealed class PrivilegeScreen : GameScreen
    {
        ...
        static string RewardIcon(string item)
        {
            ...
            if (Recipes.IsRecipe(item)) return RecipeIcon(Recipes.PartOf(item));
            return "ui.gemRed";
        }

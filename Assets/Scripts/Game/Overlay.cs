...
namespace KkomaKnight.Game
{
    ...
    public sealed class Overlay
    {
        ...
        /// <summary>
        /// T155 ⓒ · <b>T234 에서 다시 쟀다</b> — «레벨 업» 리본 뒤 빛 두 겹의 자리다. 이 사각형이 곧 <b>빛의 정사각형</b>이다
        /// (아래 호출부가 <c>sidePx = FrameW × W/100</c> 으로 한 변을 여기서 가져간다 · 폭 60.8% × 1080 = 657px = 높이 28.1% × 2337).
        /// <para>
        /// <b>왜 옮겼나(주인 «특전에서 라이트를 … 레퍼런스랑 같게? 안 된 거 같은데»)</b> — 마스크는 안 걸린 것이 맞고 그것이 의도인데(T192 예외 한 자리),
        /// 정작 다른 것은 <b>크기</b>였다. 가운데 세로띠에서 «리본 위로 뻗는 빛 높이 ÷ 화면 높이» 를 재면
        /// <b>우리 13.0%(152px/1168) ↔ 레퍼런스 19.7%(308px/1560)</b> — 레퍼런스가 1.5배 높다(`screens` run 499 ↔ <c>docs/ref/04_perks.jpg</c>).
        /// </para>
        /// <b>새 값의 셈</b> — 보이는 높이 ÷ 기하 높이 = 13.0 ÷ 15.1 = 0.861(스프라이트 둘레가 투명해서 상자보다 덜 보인다) ⇒
        /// 19.7% 를 얻으려면 기하로 <b>22.9%</b> 가 필요하다 ⇒ 빛 top = 리본 top 25.8% − 22.9% = <b>2.9%</b>.
        /// <para>
        /// ⚠ <b>그 «0.861» 은 상수가 아니었다(회차 2 실측)</b> — 이 값으로 갈아 끼운 run 503 에서 보이는 높이는 19.7% 가 아니라 <b>15.8%</b> 였다.
        /// 다만 그 회차는 짙기도 같이 낮춰서(90 → 76) 두 몫이 섞여 있다 — 빛의 바깥쪽은 원래 옅어 짙기를 낮추면 그 테두리가 통째로 안 보이게 된다.
        /// 그래서 회차 3 은 <b>짙기만 되돌리고 이 상자는 그대로 둔다</b>: 다음 측정이 «크기의 몫» 만 말하게 해서, 모자라면 그때 이 한 값만 다시 올린다.
        /// </para>
        /// <para>
        /// <b>⚠ 그런데 «1.5배로 키운다» 가 답이 아니다 — 옆으로도 같이 커지기 때문이다(스프라이트가 정사각형이다).</b> 가로를 재 보면
        /// 레퍼런스는 <b>위로 갈수록 좁아지는 가는 기둥</b>이다(리본 위 3%p 에서 폭 35% · 6%p 에서 <b>23.9%</b> · 그 위는 가운데 몇 픽셀만 남는다)인데
        /// 우리는 <b>위까지 넓은 원반</b>이다(3%p 35% · 6%p 35% · 10%p 28.7%). 지금 «폭» 은 이미 레퍼런스와 비슷하므로(둘 다 35%)
        /// 한 변을 1.29배로 키우면 높이는 맞지만 <b>폭이 45% 로 벌어져 오히려 멀어진다</b>.
        /// ⇒ 그래서 <b>조금만 키우고(21.8% → 25.0% · 1.15배) 나머지는 «위로 올려서»</b> 높이를 맞춘다.
        /// 아래끝은 27.9% — 지금(32.4%)보다 <b>훨씬 덜</b> 내려오면서도 리본 띠(25.8~30.7%) 안이라 빛은 여전히 리본 «뒤» 에서 나온다
        /// (4항 «아래로 새면 실패» 를 어기지 않는다). 한 변 25.0%(높이) = 54.1%(폭) 이라 <c>UiTextureTests</c> 의 «폭 60% 벽» 도 그대로 산다.
        /// </para>
        /// </summary>
        public static readonly Layout.R TitleGlowR = new Layout.R(22.95f, 2.9f, 54.1f, 25.0f);
        /// <summary>
        /// 그 빛의 짙기 — 레퍼런스는 가장 밝은 곳이 (234,233,231) 이지만 우리 배경은 더 밝아(어둠 α 0.85) 그대로 주면 화면이 씻긴다.
        /// <b>T234 회차 1 에서 90 → 76 으로 낮췄다가 회차 2 에서 <u>도로 90</u> 으로 올렸다 — 재 보니 그 한 줄이 높이를 갉아먹었다.</b>
        /// <para>
        /// 3항 ⓑ 의 처방(«알파를 낮추고 크기를 키우면 부채에 가까워진다»)대로 크기와 짙기를 <b>같이</b> 옮겼는데, `screens` run 503 에서
        /// 리본 위 높이가 <b>13.0% → 15.8%</b> 로 목표(19.7%)에 한참 못 미쳤다. 상자를 기하로 15.1% → 22.9% 나 키웠는데도 그렇다.
        /// 까닭은 빛의 <b>바깥쪽이 원래 옅다</b>는 데 있다 — 짙기를 0.84배로 낮추면 그 바깥 테두리가 통째로 «안 보이는» 쪽으로 넘어가
        /// <b>키운 만큼이 그대로 깎인다</b>. 즉 두 손잡이가 같은 것(보이는 높이)을 반대로 민다.
        /// </para>
        /// ⇒ <b>짙기는 되돌리고 크기만 남긴다</b>(한 회차에 한 손잡이 · 그래야 다음 측정이 «크기의 몫» 만 말한다).
        /// 부드러움은 짙기가 아니라 조각·회전이 정할 몫이고, 주인 물음의 핵심은 «레퍼런스만큼 높은가» 다.
        /// 레퍼런스는 가장 밝은 곳이 (234,233,231) 이지만 우리 배경은 더 밝아(어둠 α 0.85) 그대로 주면 화면이 씻긴다 —
        ...
        //     회차 3(light2 × 90)       =  70/255 × 90/255  = 0.097   ← 부채는 됐는데 높이가 15.8% 로 줄었다
        //     회차 4(light2 × 130)      =  70/255 × 130/255 = 0.140   ← 원반의 **6할**이라 아직 부채 쪽이다
        //   ⚠ 여기서 더 올리면(≈219) light2 가 옛 수레바퀴와 **같은 진하기**가 되어 회차 2 로 되돌아간다 —
        //     즉 이 조각들로는 «레퍼런스만큼 높게» 와 «부채처럼 옅게» 가 어느 선부터 서로를 잡아먹는다.
        //     레퍼런스는 정사각 방사형이 아니라 **위로 솟은 기둥** 그림이라 그렇다(3항 ⓒ «조각 탓이면 거기서 멈춘다»).
        ...
        public void LevelUp(BattleState G, Action<PerkDef> onPick)
        {
            ...
            if (ribbon != null)
            {
                ...
                // 빛이 화면 위 절반을 덮었다(screens run 360).
                // T234(주인 «레퍼런스랑 같게? 안 된 거 같은데») — 그 회차가 잡아 둔 상자는 «레퍼런스 빛의 bbox» 였는데,
                //   정작 그 안에 그리는 것은 **정사각형 스프라이트**라 위로 뻗는 높이가 레퍼런스의 2/3 밖에 안 됐다(13.0% ↔ 19.7%).
                //   그래서 상자를 «빛의 정사각형 그 자체» 로 다시 잡았다 — 셈은 TitleGlowR 주석에 있고, 아래끝은 오히려 올렸다(4항 «아래로 새면 실패»).

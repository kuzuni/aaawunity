...
namespace KkomaKnight.Game
{
    ...
    public static class BorderAudit
    {
        ...
        public static readonly HashSet<string> Exempt = new HashSet<string>
        {
            ...
            // res_win 의 «챕터 1» · «챕터 2 해금!» = 제목 리본 위·아래에 얹힌 **맨 글자 두 줄**이다(어둠 위에 상자 없이 · `screens` run 441 의 res_win 눈 확인).
            // 위의 «티켓 줄»·«티어 줄»·«새로고침 줄»·«합계 줄» 과 **같은 꼴**이라 같은 자리에서 뺀다 — 이름에 «줄» 이 들어가 감사 대상이 됐을 뿐이다.
            // T219 가 이 화면에 이름표를 처음 달면서(그 전엔 칸 하나뿐이라 아무것도 안 재고 있었다) 이 둘이 «처음으로» 감사에 들어와 main 이 빨개졌다(CI #446·#447 · 배포 정지).
            "챕터 줄",
            "해금 줄",
            ...
            "챕터 카드(스테이지 그림)",
            // 06 장비 화면의 공·체·실 3칸 — 위 «챕터 카드» 와 같은 까닭이다: **담개라서가 아니라 주인이 그 테두리를 빼라고 해서**
            // (2026-09-09 09:2X T309 «장비에서 공 체 실 부분에 보더 부분 빼기» · T69 «전 화면 검은 아웃라인» 보다 뒤에 온 지시라 이 칸에서는 이쪽이 이긴다).
            // 칸 조각(`ui.frameDark`)의 제 테두리는 그대로 있고, 그 위에 얹던 검은 링만 없앴다(`GearScreen.StatCell`).
            "스탯 요약줄(3칸)",
        };

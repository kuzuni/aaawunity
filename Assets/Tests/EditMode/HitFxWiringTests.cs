...
namespace KkomaKnight.Tests
{
    ...
    public class HitFxWiringTests
    {
        ...
        public void NoGameCodeStillSpawnsTheOldAdditiveHitPrefab()
        {
            ...
            // ⚠ 여유 0 — 오늘 이 셈은 **정확히 3**이다(검수 Q 실측 `9725bb30` · 파일 100개 · 3건):
            //   `BattleWorld.cs:1160`(Hit) · `:1171`(PlayerHit) · `:1192`(Counter). (`:1189` 는 주석이라 안 센다.)
            //   곧 지금이 딱 문턱이라, 뒷날 Hit·Counter 를 한 도우미로 **합치는 옳은 손질**을 하면 이 자가 거짓 빨강을 낸다.
            //   그때는 문턱을 내리지 말고(결정 930) **합친 그 도우미를 세는 쪽**으로 자를 고쳐라 — 물음은 여전히
            //   «부르는 자리가 새것을 부르나» 이지 «HitBurst 가 세 번 적혀 있나» 가 아니다.

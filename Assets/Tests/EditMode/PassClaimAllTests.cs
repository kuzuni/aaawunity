...
namespace KkomaKnight.Tests
{
    ...
    public class PassClaimAllTests
    {
        ...
        {
            var d = Load(); var s = new SaveData { PassLv = 7 };
            double gem0 = s.Gem;
            var got = Pass.ClaimAll(s, d);
            Assert.AreEqual(7, got.Count, "무료 열 1~7 = 일곱 칸");
            Assert.AreEqual(100 * 6 + 500, s.Gem - gem0, 1e-9, "100×6 + 5레벨 500 = 1,100 다이아(주인 규칙)");
            ...
            Assert.AreEqual(100 * 6 + 500, s.Gem - gem0, 1e-9, "두 번째에 재화가 안 는다");
        }

        [Test]
        public void 산_열만_받는다_유료2는_2배()
        {
            var d = Load(); var s = new SaveData { PassLv = 5 };
            Pass.ClaimAll(s, d);
            double after1 = s.Gem;
            s.PassPaid2 = true;
            var got = Pass.ClaimAll(s, d);
            Assert.AreEqual(5, got.Count, "유료2 를 사면 1~5 다섯 칸이 새로 열린다");
            Assert.AreEqual((100 * 4 + 500) * 2, s.Gem - after1, 1e-9, "유료2 = 2배(주인 «더 비싼 거는 2배»)");

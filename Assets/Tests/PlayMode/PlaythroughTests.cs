...
namespace KkomaKnight.Tests.Play
{
    ...
    public class PlaythroughTests
    {
        ...
        /// </para>
        /// <para>
        /// ⚑ <b>그 순서가 «등장 연출 중» 에도 서는지는 짐작하지 않고 <c>Overlay</c> 를 열어 확인했다</b>(T423 · 결정 1217) — 클리어 팝업은 버튼 둘을 <b>순서대로</b> 띄운다
        /// (<c>At(0.6f, b1)</c> = «광고 ×2» · <c>At(0.72f, b2)</c> = «그냥 받기»)라, 그 0.12초가 «광고만 있는 창» 이면 이 자가 조용히 광고를 누른다.
        /// <b>그런 창은 없다</b>: <see cref="UiKit.Reveal"/> 은 오브젝트를 <b>끄지 않고</b>(<c>CanvasGroup.alpha = 0</c> · <c>blocksRaycasts = false</c>) 두 버튼의 글자는 그 앞줄에서 이미 박힌다
        /// ⇒ <c>GetComponentsInChildren&lt;TMP_Text&gt;(false)</c> 가 연출 중에도 «그냥 받기» 를 집는다(게다가 아래는 집기 전에 <c>CompleteAllTweens()</c> 로 연출을 끝낸다).
        /// <b>이 문단을 지우려거든 그 두 사실부터 다시 재라.</b>
        /// </para>
        /// <para>
        ...
        /// </para>
        /// <para>
        /// 재는 것은 넷이다 — ⓐ <b>표를 빠짐없이 그 차례로 돈다</b>(<c>Run</c> 이 표를 따라갔는가 · 등록이 하나 빠지면 조용히 건너뛴다)
        /// ⓑ <b>그 표 차례가 옳다</b>(표 자체가 «P11 이 맨 끝» 인가) ⓒ 실패 0 · 끝나면 로비 · 배속 되돌림 ⓓ 빨간 줄 0.
        /// </para>
        /// <para>
        /// ⚑ <b>ⓐ 와 ⓑ 는 다른 것을 잰다 — 섞으면 ⓑ 를 안 재게 된다.</b> ⓐ 의 <c>AreEqual(want, ranIds)</c> 는 기댓값 <c>want</c> 를
        /// <see cref="Playthrough.Stages"/> <b>에서 그대로 뽑아 만든다</b> — 그래서 표를 어떻게 뒤섞어 놓아도 <c>Run</c> 이 그 표를 따라 돌기만 하면 초록이다.
        /// ⇒ <b>«P11 이 맨 끝» 계약을 잡는 것은 ⓐ 가 아니라, 그 아래 <c>want[want.Count - 1]</c> 단언 한 줄뿐이다</b>(그 줄에 붙인 ⚑ 를 보라).
        /// </para>
        /// <para>
        /// ⚠ <b>초는 단언하지 않는다</b>(결정 1196 ⑤) — 못 재 본 값으로 문턱을 박으면 애먼 빨강이 뜨고 다음 사람이 문턱을 낮춘다(결정 930).
        /// </para>
        /// <para>
        /// ⚑⚑ <b>그리고 아래 <c>Debug.Log</c> 는 CI 에서 아무도 못 읽는다</b>(결정 1200 · 1회차가 «꼬리에서 이 줄을 보라» 고 적었던 것이 틀렸다).
        /// 잡 로그는 <b>그림차 단계 → 명부 단계</b> 로 바로 넘어가고 그 사이에 유니티 콘솔 출력이 <b>한 줄도 안 실린다</b>;
        /// 아티팩트는 프록시에 막혀 있어(<c>ci_test_failures.py</c> 가 스스로 «이 목록이 워커가 읽는 유일한 자리다» 라고 적어 둔 그 까닭) 워커가 닿을 길이 없다.
        /// ⇒ <b>예산을 재는 자리는 이 자가 아니라 배포 스모크의 «시간 …s/300s 예산» 줄</b>(T406)이다 — 거기가 진짜 배포 빌드를 재는 곳이고 애초에 그 예산의 임자다.
        /// 이 줄은 에디터에서 이 자를 손으로 돌려 보는 사람에게만 쓸모가 있다.
        /// </para>
        /// </summary>
        ...
        public IEnumerator 열한_갈래를_한_판에서_이어서_돈다()
        {
            ...
            Debug.Log("[T418] " + done + "  ← 에디터에서 볼 때만 보인다(CI 잡 로그에는 유니티 콘솔이 안 실린다 · 결정 1200)");

            // ⓐ 표를 빠짐없이 그 차례로 돈다 — 「Run 이 표를 따라갔는가」만 잰다.
            //    ⚠ 기댓값 want 를 Stages 에서 그대로 뽑아 만들기 때문에, 이 단언은 표 차례가 옳은지는 **못 잰다**(표를 뒤섞어도 초록이다).
            ...
            // ⓑ 그 표 차례가 옳다 — ⚑ **이 줄이 «P11 이 맨 끝» 계약을 잡는 유일한 줄이다.** 위 ⓐ 는 못 잡는다(want 가 표에서 나오므로).
            //    쓸데없어 보인다고 지우면 P11 을 목록 가운데로 옮겨도 이 파일이 전부 초록이 된다 — 결정 1196 ③ 이 막으려던 바로 그 자리다.

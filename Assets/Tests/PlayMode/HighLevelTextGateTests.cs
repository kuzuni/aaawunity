...
namespace KkomaKnight.Tests.Play
{
    ...
    /// </para>
    /// <para>
    /// ⛳ <b>처음엔 일부러 «막는 자» 로 안 세웠고(<see cref="Strict"/> = false), 2026-09-11 T463 2회차에 켰다.</b>
    /// 그때 안 켠 까닭은 «47자가 그 줄에서 <b>실제로</b> 잘리는가 — <b>안 쟀다</b>» 였다: 재 보지도 않은 값으로 막으면 <b>모두의 배포를 세운다</b>
    /// (결정 930·1007 이 값을 치른 그 갈래 · <c>ClipStrict</c>·<c>GlyphStrict</c> 가 지나온 길과 같다).
    /// <b>그래서 이 자는 한동안 «판정» 이 아니라 «수» 로 살았고</b>, 그 수가 0 이 된 회차에 켜졌다 — 순서가 뒤바뀌지 않은 것이 이 절이 지킨 것이다.
    /// </para>
    /// <para>
    /// ⛳ <b>고침은 이 절이 안 했다 — 두 절이 나눠 했다.</b> ⑤ 의 셋(사다리 늘리기 · 지수 꼴 · 그 줄만 좁히기) 가운데
    /// <b>지수 꼴</b>을 T463 이 <c>ShortNum.cs</c> <b>한 곳</b>에서 했고(사다리 꼬리에 상한 · 47자 → <b>15자</b>),
    /// 남은 넘침 한 건(«+1»)은 <b>T460</b>(워커 P)이 했다. 잰 수로는 <b>3 → 1 → 0</b> 이고, <b>어느 한 절도 혼자서는 0 을 못 만들었다</b>.
    /// </para>
    /// </summary>
    public class HighLevelTextGateTests
    {
        /// <summary>
        /// 켜면 «넘침·하한 미달 0» 을 <b>막는 자</b>로 단언한다. <b>2026-09-11 T463 2회차에 켰다</b>.
        /// <para>켤 자격이었던 것: T455 ⑤ 의 고침이 서고, 이 자가 찍는 표에서 <b>07 의 넘침·하한 미달이 0</b> 이 된 회차.</para>
        /// <para>
        /// ⚑ <b>켠 근거는 어림이 아니라 이 자가 제 손으로 찍은 수</b>다(<c>git show origin/screens:highlevel.json</c> · 런 <b>1101</b>):
        /// <c>cost_line.shown «1.56e33/1.56e33» · shownLen 15</c>(옛 <b>47</b>) · <c>counts {floorBad 0 · bestFitBad 0 · clipped 0}</c>.
        /// 3 → 1 → 0 으로 내려온 두 걸음에 임자가 각각 있다: <b>3 → 1</b> 은 T463 1회차(<c>ShortNum</c> 사다리 꼬리 상한),
        /// <b>1 → 0</b> 은 T460(워커 P)의 «+1» 이다 — <b>내 절만으로는 0 이 안 됐다</b>.
        /// </para>
        /// <para>
        /// ⚠ <b>켠 뒤의 빨강은 «이 자가 틀렸다» 가 아니라 «그 줄이 다시 넘친다» 는 뜻</b>이다 — 문턱을 낮추거나 이 값을 <c>false</c> 로 돌리는 것으로 끄지 마라(결정 930·1007·1276).
        /// 고칠 곳은 <b>글자를 내는 쪽</b>(<c>ShortNum</c> · 그 줄의 <c>TextKind</c>·칸 폭)이고, 이 자가 찍는 표가 어디가 넘쳤는지 이름으로 말해 준다.
        /// </para>
        /// </summary>

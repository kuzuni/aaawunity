...
namespace KkomaKnight.Tests.Play
{
    ...
    public class UiSmokeTests
    {
        ...
        public IEnumerator LobbyPrivilegeAndSeasonPass()
        {
            ...
            foreach (var n in new[] { "Banner", "PassName", "SeasonEnds", "LevelBadge", "Track",
                                      "Col:free", "Col:paid1", "Col:paid2", "Line",
                                      "ClaimAllBtn", "BuyBtn:1", "BuyBtn:2", "BackBtn" })   // T304 — «방랑자의 보상» 탭 · T322 — 구간 띠·«💎100» 배지는 주인이 지웠다 · T462 — 진행 바·안내 띠도
                Assert.IsNotNull(UiKit.Find(sp, n), "시즌 패스 조각 «" + n + "»(표 ㊼)");
            // T322(주인 «다이아 100 이라고 써 있는 부분 없애기») · T462(주인 2026-09-12 «패스에 이 부분 없애기» — 메달·진행 바·안내 띠) —
            //   지운 것이 **정말 없는지**도 잰다. 목록에서 이름만 빼면 «안 그린다» 가 아니라 «안 본다» 다(T184 가 값을 치른 자리).
            //   ⚠ `Notice` 는 `BuildNotice()` 가 **남아 있고 안 불리는** 꼴이라 더욱 그렇다 — 누가 그 줄을 되살리면 목록에서 뺀 것만으로는 아무도 안 운다.
            foreach (var n in new[] { "SegBand", "SegBadge", "ProgressBar", "Medal", "Notice" })
                Assert.IsNull(UiKit.Find(sp, n), "지운 조각 «" + n + "» 이 다시 서 있다(주인 T322·T462)");

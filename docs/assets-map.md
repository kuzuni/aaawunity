...
| prefabs | `ui.chestOpen` | `Assets/Layer Lab/GUI Pro-MinimalGame/Theme_Light/Prefabs/Prefabs~DemoScenes/Shop_Chest_Open.prefab` | root 8655925205862286414 | 장비 소환 결과 팝업 (주인 지정 Shop_Chest_Open 그대로 · T9 확인) — Title_01 리본 = «상자 N회» · Image_Chest = chest.*.open · 프리팹의 보상 칸(ItemFrame_01 · y+217) 자리에 얻은 장비 격자(칸 = ListItem_EquipMent · 4열 · 세로 가운데) · Text_TouchContionue = 터치하면 닫기 |
| prefabs | `ui.lobby` | `Assets/Layer Lab/GUI Pro-MinimalGame/Theme_Light/Prefabs/Prefabs~DemoScenes/Lobby_Default.prefab` | root 4248817929678912515 | 로비 (T34 · 2026-09-06 주인 지시 «UI 는 docs/ref/01_lobby.jpg 구도» — Lobby_Default 는 부품 창고) — 쓰는 조각: Background(+Deco · 초록 틴트) · Button_Menu(≡ → 설정 · LobbyMenu 자리) · Title_LineDeco_01_Blue(챕터 제목 · LobbyChapTitle) · Tab_01_BottomFlushMenu(탭 바 · TabBar). 끄는 조각: UserInfo_01·ResourceBar_Group(상단 바는 TopBar 헬퍼가 ui.userInfoSlider·ui.resourceBar 로 다시 세운다) · Group_Left/RightButtons(사이드 아이콘 3+3 은 ui.frameDark + 아이콘·라벨) · SampleImage_Map(카드는 ui.frameDarkBorder + 테마 Environment 조각) · Button_03_Red(START 는 ui.btnStartOrange) · ChatBox · 부제 |

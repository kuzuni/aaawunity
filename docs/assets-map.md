...
| prefabs | `ui.chestOpen` | `Assets/Layer Lab/GUI Pro-MinimalGame/Theme_Light/Prefabs/Prefabs~DemoScenes/Shop_Chest_Open.prefab` | root 8655925205862286414 | **소환(뽑기) 결과 창 = 이 조각 그대로**(T95 · 주인 2026-09-07 «소환 결과 창이 Shop_Chest_Open 이거로 돼야 하는데 안 됐더라 · 애니메이션 찰지게») — 조각: Background(어둠)+Pattern · Chest 묶음(Shadow · Image_Chest = 우리 `chest.*.open` 으로 갈아 끼움 · Light · SampleEffect 4) · Text_TouchContionue(«탭하여 닫기»). 조각에 격자가 없어 얻은 장비 칸(ListItem_EquipMent 4열)·제목·안내 줄만 상자 «위» 에 얹는다 · 연출 = 흔들림 → 빛 폭발 → 칸 하나씩(오버슛) → 최고 등급 한 번 더(`ShopScreen.ChestResult`) |

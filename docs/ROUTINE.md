...
2. **처방(결정 344)**: 배치 표(⑰)·`Layout` 상수는 **한 칸도 안 건드린다**(`ui_score` 25 = 10.0/10 이고 표는 레퍼런스 실측이라 옳다) — 버튼만 줄의 위쪽 **72%**(`EventsScreen.TabBtnH`)를 쓰게 한다(버튼 5.1%p ≈ 레퍼런스 5.3%p · 아래 여백 2.0%p).
...
1. **처방(결정 345)**: 아이콘 키 → 테두리 키 **표**(`EventsScreen.RewardFrames` · 표에 없으면 초록)를 두고 `IconRow(..., frameByIcon: true)` 가 칸마다 조각을 갈아 끼운다(T103 2항 «등급은 색 변형 교체만» 과 같은 문법 · tint 로 물들이지 않는다).
...
1. **무엇이 어긋났나**: 우리 `UiKit.Popup` 은 `ui.popup` 조각(크림)으로 상자를 세운다. T111 ⓑ(«검정 글씨 → 흰 글씨»)가 들어온 뒤로 **크림 상자 위 흰 글자**가 되어 07 이름줄(«치명 무기») 같은 자리가 약해 보인다. T121 은 이것을 «글자 문제» 로 보고 12 설정에서 **줄 띠만** 어둡게 해 국소로 막았는데(결정 316), 07 을 보고 나니 **어긋난 것은 글자가 아니라 상자 색**이다(결정 342).

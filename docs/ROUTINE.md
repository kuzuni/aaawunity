...
### T99 — 던전: 티켓 하루 2개 보충 + 광고/다이아 구매 · 지옥문·원정 **보상 수치 확정** (주인 2026-09-07 · 수치는 주인이 준 값 그대로) — **🔄 코드 push(`9e4c6b9` · sess-2041-14225 · 워커 C · 로컬 게이트 전부 초록 · 배포 스모크 초록)**: 수치는 새 표 `Assets/KkomaKnight/dungeon.json`(«data.dungeon» · 코드 상수 0 · `data/*.json` 불변) · `Core/Dungeon`(Roll = «2 미만이면 2 로» · 광고/다이아 던전별 하루 1회 · Ready) · 세이브 네 필드 · 세부 팝업의 두 버튼 교체(못 쓰면 알파 0.5 + 이유 토스트 = 결정 263) · 보상 칸을 표에서(지옥의 문 네 칸이라 표 ⑪ 불변) · 펫알 지갑은 안 만들었다(던전 전투가 없다 = 결정 264) · **확인 = 그 커밋 CI 의 EditMode `DungeonTicketTests` 7 + PlayMode `DungeonTicketPlayTests` + screens 20·21**

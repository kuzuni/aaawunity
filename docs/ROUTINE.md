...
### T105 — 장비 칸의 **다이아 배지 = 부위 아이콘**(세트 아이콘 아님) + 장착 슬롯 6칸에 부위 아이콘 기본 표시 (주인 2026-09-07 · 화면만 · T88 과 짝) — **🔄 코드 push(`7bb8436` · sess-1928-29105 · 워커 F · 로컬 게이트 전부 초록 · 배포 스모크 초록)**: `GearLook.PartIcon` 신규(**새 에셋 0 · 카탈로그 불변** — 이미 있는 `gi.<부위>.crit` 재사용) · 칸 배지와 슬롯 아이콘을 부위 아이콘으로 · 슬롯 아이콘은 **빈 칸에도 흐리게 늘 켜 둔다**(α 0.45 · 결정 281 = 빈 칸 «+»·링은 안 건드려 T69 게이트 불변) · 열 순서 `ColLeft = weapon·neck·glove` / `ColRight = helm·armor·boot`(T88 역할 묶음과 같다 · 배치 표 불변). **확인 = 그 코드를 담은 첫 완주 런의 `UiSmokeTests.GearScreenDetailSlotAndEquip` Passed + screens 06 PNG** · 결정 280·281.

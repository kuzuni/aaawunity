...
1. **대상**(전부 `Overlay.cs`) — `Devil`(악마의 거래 · 410) · `Angel`(천사의 축복 · 429). **같은 흐름의 뒷 팝업 `DevilGift`(악마의 선물 · 420) · `Blessed`(축복 강화 · 439)도 같이 맞추길 권한다**(주인이 «통일» 이라 했고 한 흐름에서 판이 있었다 없어지면 더 어색하다). **쉼터(`Rest` · 392)** 는 주인이 안 불렀지만 같은 «이벤트 팝업 3형제» 다 — 워커가 정해 같이 하거나 남기고, 어느 쪽이든 «워커 결정 기록» 에 한 줄.

...
### T93 — 특전 행(카드) 디자인을 데모 프리팹 `Play_Perk_Selection_02` 와 **같게** · 색은 회색·노란색·빨간색 (주인 2026-09-07 · 화면만 · 엔진 불변) — **🔄 코드 push(`8a2a9b1` · sess-2148-1691 · 워커 L · 로컬 게이트 전부 초록 · 배포 스모크 초록) · 남은 일 = 확인뿐**
...
**✅ `T96-menu` ⓐ·ⓔ 는 코드가 들어갔다 — `e5677c2`(sess-2050-1710 · 워커 L · 로컬 게이트 전부 초록 · 배포 스모크 초록).** 프리팹 줄이 넷이라 **줄을 두 번 복제**하고 판 높이만 6/4 로 늘렸다(크기·아이콘·글자 자리는 프리팹 그대로 · 결정 267) · 항목 순서 = 주인이 부른 순서(우편함 · 설정 · 데일리 기프트 · 퀘스트 · 출석 · 특권) · **로비의 사이드 기둥 둘(특권 · 출석/데일리 기프트/퀘스트)은 지웠다**(중복 제거 · `ref-layout` ① 두 행 삭제 · ㉜ 표 신설) · 알림은 `Core/Notify` 한 곳이 정하고 **판정이 있는 항목만** 켠다(결정 268). **우편함은 `LobbyMenu.Mailbox` 훅 자리** — `T96-mail` 워커가 그 한 줄만 채우면 메뉴가 그대로 연다. 확인 = 이 커밋을 담은 첫 완주 런의 PlayMode `LobbyMenuTests` 3 + 회귀 0 + `screens` 01 PNG.

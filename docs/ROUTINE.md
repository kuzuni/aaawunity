...
- **(2026-09-07 · 03:5X UTC) ⚑⚑ 주인 지시 — 투사체 두 가지: ⓐ **도끼·창이 발사되고 바로 안 날아간다** → 쏘는 즉시 날아가게. ⓑ **창은 누워서 일자로 가야 하는데 비스듬한 각으로 날아간다** → 수평으로 고쳐라.** → **T86** 등재(§2 · 화면 연출만 · 엔진 판정 불변 · 루틴이 잡는다).
...
8. 테스트(PlayMode): ⓐ 킬 특전으로 쏜 도끼·창이 **발사 뒤 0.1초 안에 스폰 위치보다 눈에 띄게 전진**한다(보류 중에도) ⓑ 창 오브젝트 `rotation.eulerAngles.z` ≈ 0(±2° · 스프라이트 보정값을 상수로 두면 그 상수 기준) ⓒ 화살·도끼(회전)·검기는 종전 그대로 ⓓ 시드 골든·`BattleWorldTests` 회귀 0 ⓔ `LogAssert.NoUnexpectedReceived`.

...
### T64 — WebGL 오디오: BGM(bgm.*) 이 «NotSupportedError: Failed to load because no supported source was found» · SFX(snd.*) 가 «EncodingError: Unable to decode audio data» + «Loading FSB failed for audio clip "click"» (T59/T60 스모크에서 발견 · 워커 G 등재 2026-09-06 · 제약 없음) — **🔄 코드 완료 `553ab73`(워커 A): 20개 클립 Vorbis → PCM(SFX 17 = PCM+DecompressOnLoad · BGM 3 = PCM+22.05kHz+모노 · 17.3MB→5.3MB) · 근거 = 배포 빌드 framework.js 실측(`_JS_Sound_Load` 는 128KB 미만이면 decodeAudioData 강제 · 그 이상은 `<audio>`+audio/mp4 · `_JS_Sound_Load_PCM` 만 코덱 무관) · 회귀 게이트 `tools/check_audio_webgl.py`(§3·ci.yml) · 결정 128 · 남은 일 = 그 CI 의 배포 스모크 `AUDIO⚠` 0 확인 → `--strict-audio` 로 전환하며 ✅**

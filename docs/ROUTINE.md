...
### T110 — 클리어 팝업: **움직이는 패턴 제거 · 타이틀 `SampleEffect` 회전 · 등장할 때 폭죽 파티클** + **골드 «+49 G» 팝 텍스트 삭제** (주인 2026-09-07 · 연출만) — **🔄 코드 push(sess-1957-28503 · 워커 D · 로컬 게이트 전부 초록)**: ⓐ 골드 «+N G» 팝 삭제(소리는 유지) ⓑ 승리·사망 팝업 `DimPattern` 제거(레벨업 3택은 유지 · 결정 286) ⓒ 제목 `SampleEffect` 시계방향 14s 무한 회전 ⓓ 프리팹 `SampleEffect_Confetti` 를 되살려 좌우 두 번 «펑»(새 에셋 0 · 결정 287) · **확인 = 그 커밋의 CI PlayMode(`UiTextureTests` 뒤집은 단언 + `BattleWorldTests` 골드 팝 0) + 주인 폰**

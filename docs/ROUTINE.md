...
> **실측 결과**: 조각·색 이름은 **이미 프리팹과 같았다**. 진짜 결함은 `screens` run 173 의 04 PNG 를 확대해서 나왔다 — `UiKit.Desaturate` 가 프레임 전체를 훑어 **일반 등급의 «등급 탭» 이 몸통에 묻혀** 통짜 회색 판으로 보였다(결정 284). 고침 = `Palette.PerkTabColor` 로 **탭에만** 등급색 + 설명 글자 칸 좌우를 프리팹 실측(0.2239~0.9649)으로. **프리팹과 일부러 다른 곳**(카드 사이 간격 107↔38px · 카드 높이 11.46↔11.0% · 두 줄↔한 덩어리)은 «화면 위 자리는 레퍼런스 표 ⑦ 이 정본» 원칙(결정 285)과 주인 2026-09-05 «이름 금지» 때문이고 PROGRESS 대조표에 적었다.
...
### T110 — 클리어 팝업: **움직이는 패턴 제거 · 타이틀 `SampleEffect` 회전 · 등장할 때 폭죽 파티클** + **골드 «+49 G» 팝 텍스트 삭제** (주인 2026-09-07 · 연출만)

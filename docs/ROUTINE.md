...
>
> **2단계 · 장비·대장간 묶음(`T90-gear`) ✔ 코드 push(`3c8bfe8` · sess-1928-29105 · 워커 F · 로컬 게이트 전부 초록 · 배포 스모크 초록)** — 06·07·08 을 전수로 훑었더니 **고칠 문구가 하나도 없었다**(장비 스탯 줄 = 절대값 · 슬롯/합성 안내는 이미 % · 세트 옵션은 1단계가 처리). 그래서 ⓐ 게이트가 **08 대장간**을 열게 하고(재료 한 칸 눌러 안내 문구까지) ⓑ `StrictScreens` += 02·06·08 ⓒ **`PercentAudit.Strict = true`**(3항의 «표가 0 이 되면 strict» · 근거 = CI #173 의 «% 빠진 줄 0» + 화면 코드 전수 확인 + 데이터 전수 테스트) ⓓ EditMode `StatTextTests` 에 **표 밖의 이름**을 잡는 테스트 1개(기존 두 테스트는 표에 든 12개 이름만 본다). **확인 = 그 코드를 담은 첫 완주 런의 `PercentGateTests` Passed + 로그 «[PercentGate] … strict=True … 0»** · 결정 253~255.
>

...
>
> **🔄 (02:3X UTC · 워커 F · sess-0231-31128) `T96-profile` **2단계(이름 바꾸기)** 코드 push — 남은 것은 확인이다.** 1단계가 남긴 벽을 `UiKit.Adopt` 에서 풀었다(결정 346): TMP 입력칸을 **먼저 걷어 내** 쪽지를 남기고, TMP_Text 를 전부 uGUI Text 로 바꾼 **뒤** 그 쪽지로 uGUI `InputField` 를 붙인다 — 자리·크기·글자 자리는 프리팹 그대로고 입력칸이 없는 조각은 첫 줄에서 빠져나가 **다른 화면은 0줄**이다. 규칙은 순수 C# `Core/Nickname`(2~12자 · 기본 «꼬마기사»)이고 **한도는 주인 지목 조각 실측**이다(«/12» · «Enter at least 2 characters.» · 결정 347). **입구는 아바타 팝업의 제목 줄**(`NickBtn`) — 제목이 «아바타» 대신 지금 내 이름을 보여 주고 누르면 이름 팝업이 열린다(조각을 하나도 안 더한다 · 결정 348). 세이브 `Nick`(옛 세이브 호환 · 버전 불변) · 아레나 시상대 «나» 줄이 그 이름으로 선다(기본값이 종전 이름이라 안 고치면 화면 불변) · `ref-layout` **㉟**(프로필 팝업 둘) 신설. 확인 = 그 커밋 CI 의 PlayMode `ProfileTests` 2 · EditMode `NicknameTests` 7 + 회귀 0 + 주인 폰.
>

...
> 이 레포는 **aaaw 의 HTML 게임을 유니티(2022.3 LTS)로 이식**하는 곳이다. 게임 규칙·수치는 aaaw 가 정본이고 여기서는 **바꾸지 않는다**.
...
- **(2026-09-05 · 착수 지시)** aaaw 의 «꼬마기사 키우기» 를 이 레포에 유니티로 이식하라.
  ...
  - 유니티 에디터도 MCP 도 없다 — C#·씬·프리팹·.meta·ProjectSettings 를 **전부 텍스트로** 쓴다. 유니티 2022.3 LTS 최신 패치 고정.
...
- 브랜치는 `main` 하나다. 각 단계가 끝나면 main 에 커밋·푸시하고 PROGRESS 에 «무엇을 확인하면 되는가» 한 줄을 적는다.

## 2. 작업 목록 (순서 고정 — lock ID = 아래 번호)

> 주인이 정한 5단계다. 앞 단계의 산출물 위에 다음 단계가 얹힌다 — **T2 가 끝나기 전에 T3 을 선점하지 않는다**(T4 는 T2 와 파일이 겹치지 않아 병렬 가능 · «범위» 열 참조).
...
범위: `Assets/Scripts/Game/Lobby*.cs` · `Gear*.cs` · `Forge*.cs` · `Shop*.cs` · `Save*.cs`
...
2. 로비(CHAPTER N · 디오라마 카드 · ◀▶ · START · 하단 5탭(상점·장비·전투·카드🔒·도전🔒)).
...
dotnet test tools/dotnet/Tests/KkomaKnight.Tests.csproj -c Release --no-build   # 순수 C# 테스트

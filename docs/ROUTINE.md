...
### T64 — WebGL 오디오: BGM(bgm.*) 이 «NotSupportedError: Failed to load because no supported source was found» · SFX(snd.*) 가 «EncodingError: Unable to decode audio data» + «Loading FSB failed for audio clip "click"» (T59/T60 스모크에서 발견 · 워커 G 등재 2026-09-06 · 제약 없음) — **🔄 회차 1 `553ab73`(PCM) = 실패(유니티가 WebGL 빌드에서 PCM 무시 · 소스 Vorbis 그대로 · .data +117KB · loadType 만 반영) → 회차 2 `2dc76dc` = 20개 전부 AAC(7) · 근거 = 배포 빌드 framework.js 실측(`_JS_Sound_Load` 는 128KB 미만이면 decodeAudioData 강제 · 그 이상은 `<audio>`+audio/mp4 · `_JS_Sound_Load_PCM` 만 코덱 무관) · 회귀 게이트 `tools/check_audio_webgl.py`(§3·ci.yml) · 결정 128 · 남은 일 = 그 CI 의 배포 스모크 `AUDIO⚠` 0 확인 → `--strict-audio` 로 전환하며 ✅**

...
- **(2026-09-06 · 09:2X UTC) ⚑⚑ 주인 지시 — 특전 설명은 «트리거: 내용» 꼴로: «처치 시: 33% 확률로 …» · «피격 시: …» · «3타마다: …».** → **T53** 등재(§2 · 표시 시점 변환 · perks.json 불변 · T52 뒤 · 루틴이 잡는다).
...
> T1~T5(주인이 정한 5단계)는 끝났다. **지금 열린 작업은 T17~T53**(T49 = 팝업 등장 연출 DOTween 순서대로 · T50 = 킬 뒤 원래 걷기 속도 · T51 = 대시도 공격 모션 뒤 ×5 + 사망 펑 이펙트 제거 · T52 = 특전 글자 한 색 · T53 = 특전 설명 «트리거: 내용» 표기 · 2026-09-06) — **T34~T44 = UI 를 `docs/ref` 레퍼런스 구도로(2026-09-06 · 최우선 · «프리팹 그대로» 계열 지시를 대체 · T25 흡수 · T27·T30·T32 폐기)**. 이전 묶음: T17~T33(T12~T16 은 워커가 먼저 쓴 번호 · 내 T13~T16 은 T20~T23 으로 정정) (T12 = 콘솔 에러 수정 · 최우선 · T13 = 특전 미리보기 줄 비례 · T14 = 전투 캐릭터 크기·공격 애니·사망 모션 · T15 = 프리팹 스폰 PanelView 예외 · 콘솔 에러라 최우선 · T16 = T14 의 CI #39 빨강 후속) — 같은 파일을 만지는 것은 아래 «순서» 대로(앞 번호의 lock 이 사라지고 PROGRESS 행이 ✅ 가 된 뒤에 잡는다). 겹치지 않는 것은 병렬 선점 가능.
...
### T34 — 로비 = `docs/ref/01_lobby.jpg` 구도 (UI 레퍼런스 · 최우선) ✅ (완료 · `d6d1411` · 상단 재화 바 = `TopBar` 헬퍼(Screens.cs) — T37·T40·T42·T43 은 `TopBar.Build(App, root)` 한 줄로 · 사이드/배너/모서리 버튼은 `LobbyScreen.OnSide(key)` 훅 · PROGRESS 참조) · **코드 ✅ · 비평은 T47 에서(8.0 미만이면 다시 열림)**
...
### T35 — 전투 HUD = `02_battle.jpg`·`03_battle_enemy.jpg` 구도 + **HP·실드 바(주인 강조)** (T33 뒤) ✅ (완료 · `4773d23` · CI #56 · PROGRESS 참조) · **코드 ✅ · 비평은 T47 에서(8.0 미만이면 다시 열림)**
...
### T37 — 장비 화면 = `06_gear.jpg` 구도 (T17 뒤 · T25 흡수) · **✅ 조건 = 비평 ≥ 8.0/10(§5)** — 코드 완료(`e031e1c` · CI #60 · 조각 재조립 · 비평 대기 · PROGRESS 참조)
...
### T38 — 장비 세부 팝업 = `07_gear_detail.jpg` 구도 (T37 뒤 · T27 대체) · **✅ 조건 = 비평 ≥ 8.0/10(§5)** — 코드 완료(`7346617` · CI #63 · 비평 대기 · PROGRESS 참조)
...
### T39 — 대장간 = `08_gear_fuse.jpg` 구도 (T37 뒤) · **✅ 조건 = 비평 ≥ 8.0/10(§5)** — 코드 완료(`48d05a6` · CI #66 · 무대·결과 슬롯·액션바·인벤·뒤로 = 표 ⑥ · 이름표 ⑥ 10행 · 비평 대기 · PROGRESS 참조)
범위: `Assets/Scripts/Game/ForgeScreen.cs`
...
### T40 — 상점 = `09_shop_1.jpg`·`10_shop_2.jpg` 구도 · **✅ 조건 = 비평 ≥ 8.0/10(§5)** — 코드 완료(`c922d5a` · CI #64 · 세로 스크롤 한 화면 · 이름표 ⑤ · 비평 대기 · PROGRESS 참조)
...
### T41 — 설정 팝업 = `12_settings.jpg` 구도 (T29 뒤) · **✅ 조건 = 비평 ≥ 8.0/10(§5)** — 코드 완료(`b0bf87d` · CI #61 · 표 ⑨ 신설 · 비평 대기 · PROGRESS 참조)
...
### T42 — 펫 탭 = `13_pet.jpg` 구도 + 펫 세부 = `14_pet_detail.jpg` (껍데기 · T32 대체) · **✅ 조건 = 비평 ≥ 8.0/10(§5)** — 코드 완료(`66818af` · CI #67 · 표 ⑩·⑪ 신설 · 비평 대기 · PROGRESS 참조)
...
### T43 — 던전 탭 = 던전 `20`·`21` + 아레나 `22`~`26` 껍데기 (T30 대체) · **✅ 조건 = 비평 ≥ 8.0/10(§5)** — 코드 완료(`6acbcdb` · CI #70 · EventsScreen = 페이지 4 + 팝업 3 · 표 ⑫~⑱ 신설 · 이름표 전부 · 비평 대기 · PROGRESS 참조)
...
### T44 — 로비 사이드 팝업 껍데기 6종 = `11`·`15`·`16`·`17`·`18`·`19` (T34 뒤) · **✅ 조건 = 비평 ≥ 8.0/10(§5)** — 코드 완료(sess-0827-4159 · 워커 F · 표 ⑲~㉔ 신설 · `LobbyPopups.cs` · 비평 대기 · PROGRESS 참조)
...
### T47 — 로비(T34) · 전투 HUD(T35) 비평 회차 (T46 뒤)
...
1. 변환 규칙(`desc` 앞머리의 트리거 구를 떼어 «트리거: 나머지» 로): «처치 시 X» → «처치 시: X» · «피격 시 X» → «피격 시: X» · «공격 시 X» → «공격 시: X» · «반격 시 X» → «반격 시: X» · «N타마다 X» → «N타마다: X» · «실드가 있으면 피격 시 X» → «피격 시(실드 있을 때): X»(또는 «실드 있을 때 피격 시: X» · 워커 결정) · «실드가 0 인 동안 X» → «실드 0 일 때: X» · «보유 특전 하나당 X» → «특전 하나당: X». 트리거가 없는 상시 능력치(«공격력 +30%» · «방어력 +8%» · «가시갑옷 +100%» 등)는 **그대로**(«상시:» 를 붙일지는 워커가 정하고 결정 기록 — 기본은 붙이지 않음).
2. 확률 표기는 원문 그대로 «33% 확률로»(주인 원문의 «33퍼» 는 말투) · 콜론 뒤 한 칸 띄움 · 콜론 앞 트리거 구는 굵게 하지 않는다(T52 «한 색» · 굵기도 통일).
3. 구현은 정규식/접두어 표 한 곳(정적 헬퍼) — 100개 전수를 EditMode 테스트로 돌려 «트리거 구가 있는 71개는 전부 콜론이 붙고, 없는 29개는 원문 그대로» 를 단언(표는 테스트 안에 기대값 목록으로 · 새 특전이 생기면 테스트가 잡는다).

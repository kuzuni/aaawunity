...
### T64 — WebGL 오디오: BGM(bgm.*) 이 «NotSupportedError: Failed to load because no supported source was found» · SFX(snd.*) 가 «EncodingError: Unable to decode audio data» + «Loading FSB failed for audio clip "click"» (T59/T60 스모크에서 발견 · 워커 G 등재 2026-09-06 · 제약 없음) — **🔄 회차 1 `553ab73`(PCM) = 실패(유니티가 WebGL 빌드에서 PCM 무시 · 소스 Vorbis 그대로 · .data +117KB · loadType 만 반영) → 회차 2 `2dc76dc`(AAC) = 실패(형식은 빌드가 무시 · loadType 만 반영) → **회차 3 `9ed1c7a` = 유니티 오디오 우회**(StreamingAssets 원본 .ogg 20개 + `AudioManager.LoadStreamed` 가 `UnityWebRequestMultimedia` 로 받아 카탈로그 클립보다 우선 사용 · 런타임 WebGL 분기 · 에디터·안드로이드 불변) · 게이트 `check_audio_webgl.py` 는 «카탈로그 키 ↔ StreamingAssets 원본 존재» 검사로 교체 · 근거 = 배포 빌드 framework.js 실측(`_JS_Sound_Load` 는 128KB 미만이면 decodeAudioData 강제 · 그 이상은 `<audio>`+audio/mp4 · `_JS_Sound_Load_PCM` 만 코덱 무관) · 회귀 게이트 `tools/check_audio_webgl.py`(§3·ci.yml) · 결정 128 · 남은 일 = 그 CI 의 배포 스모크 `AUDIO⚠` 0 확인 → `--strict-audio` 로 전환하며 ✅**
**🔄 회차 4 `92c0b7b`(sess-1913-2015 · 워커 E) — 코드는 끝났고 lock 은 반납했다(19:4X · 남은 일이 «확인» 뿐이고 그 확인이 **남의 결함(T91)** 에 걸려 있어 내가 쥐고 있을 이유가 없다 — 배포가 풀린 뒤 아무 워커나 그 런의 `build-webgl` «배포 스모크» 초록 + gh-pages 갱신을 보고 이 행과 PROGRESS T64 행을 ✅ 로 닫으면 된다). 주인 종결 지시(04:3X) 이행:** 스모크에서 오디오 문구를 판정 밖으로(결정 219) + 회차 3 의 ogg 스트리밍 경로 끔(결정 220 · 유니티 네이티브가 거부하는 것이라 `streamAudio=false` 로도 안 된다 · 클립 0/20 이라 소리에 보탬 0) + headless 가 유독 못 읽는 까닭 실측(결정 221 · playwright chromium 은 `audio/mp4`·`audio/aac` 에 코덱이 없다). `tools/check_audio_webgl.py` 는 주인 지시대로 그대로. **✅ 조건 = 그 커밋 CI 의 배포 스모크 초록 + gh-pages 갱신 + 주인 폰 소리(정본).**
...
### T90 — 표기: 퍼센트로 써야 할 값에 **%** 를 전부 붙인다 («회피 +8» → «회피 +8%») (주인 2026-09-07 · 전 화면 · 문구만) — **🔄 1단계(비율 스탯 표 + 표시 함수 훅 + 감사) 코드 push(`1f6dacb` · sess-1932-24302 · 워커 K · 로컬 게이트 전부 초록 · 배포 스모크 초록)**
...
>
> **2단계(남은 일 · 화면 묶음 워커 몫)**: 화면 코드가 문자열을 **직접** 이어 붙이는 자리(GearUi 스탯 줄 · Overlay · ShopScreen · EventsScreen · 강화/합성 안내 · 토스트)는 지금 전부 다른 워커 lock 안이라 손대지 않았다 — 그 커밋이 든 CI 로그의 **«[PercentGate]» 표**(`Game/PercentAudit` · **보고만**)에 화면·경로·조각이 찍히니, 자기 화면 묶음 워커가 `StatText.Signed` 로 바꾸고 표가 0 이 되면 `PercentAudit.Strict` 를 켠다(`TextAudit.ClipStrict` 가 밟은 길 그대로).
>
...
**`T96-loading` ⓓ 로딩 화면**: 앱 부팅(`Bootstrap`/`App`)에서 데이터·에셋 로드 동안 `Title_Loading` 프리팹을 띄운다(진행 바 = 실제 로드 진행 · 끝나면 로비). WebGL 첫 로딩과 겹치지 않게(유니티 로더가 끝난 뒤 우리 로딩) · 최소 표시 시간 0.3s 정도로 깜빡임 방지.
...
### T97 — 탐험(방치·오프라인 보상) + 빠른 탐험(광고) — 레퍼런스 `30_expedition.jpg` · `31_expedition_fast.jpg` (주인 2026-09-07)
...
### T98 — 로비 → **챕터 보상(Chapter Chest)** 페이지 — 레퍼런스 `32_lobby_clear.jpg` (주인 2026-09-07)
...
### T99 — 던전: 티켓 하루 2개 보충 + 광고/다이아 구매 · 지옥문·원정 **보상 수치 확정** (주인 2026-09-07 · 수치는 주인이 준 값 그대로)
...
### T101 — 던전 팝업(20): 카드 **간격 벌리기** · **«준비 중» 카드 삭제** · 테두리를 **카드 전체를 감싸는 직사각형 링**으로 (주인 2026-09-07 · 화면만)
...
### T102 — 던전 세부 팝업(21): 빨간 **DecoLine 제거** · **그림 띠(Pic)를 상단 타이틀 바에 딱 붙이기** (주인 2026-09-07 · 화면만)
...
### T101 — 설정 팝업(12): **뒤 어둠이 화면을 다 안 덮는다** + **«탭하여 닫기» 가 하단 탭 바와 겹친다** (워커 실측 등재 2026-09-06 20:0X · 화면만 · 배치 표 불변)
...
5. 게이트 + PROGRESS T101 행 + 완료 기록(확인 = CI + `screens` 12 PNG 눈 확인 = 탭 바·재화 바가 어두워졌나).

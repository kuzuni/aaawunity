...
> **묶음 진행: `T69-lobby` ✅ 코드 `8cc90a7`(워커 A · 로비 기둥·배너·챕터 카드 + 재화 pill·HUD pill = 캡슐 조각 `fr.pillBorder` 신설 · `01_lobby` strict · 감사 예외 목록 `BorderAudit.Exempt` 신설 · 확인 = 그 CI 의 `BorderGateTests`) — 결정 149 가 남긴 «둥근 pill» 도 이걸로 닫았다. 남은 묶음: 장비·대장간·상점·펫·던전/아레나·로비 팝업·설정·결과 팝업·특전 카드 + 마지막에 하단 탭 바(전 화면 공용 프리팹).**
...
### T77 — 데일리 기프트(`17_daily_gift.jpg` · T44 껍데기 → **동작하는 기능**): 광고 누적 1회 = 다이아 100 · 2회 = 200 · 3회 선물 = 300 · 6회 선물 = 300 · 매일 초기화 (주인 2026-09-07 · 새 시스템 주인 지시 · T44 뒤 · 제약 없음) — **🔄 코드 push(워커 B · sess-1529-26671)**: `dailyGift.json` + `Core/DailyGift`(순수 C# · EditMode 13) + `SaveData` 4필드(버전 유지 = 결정 154) + `LobbyPopups.DailyGift` 실동작(버튼 4상태 · 모의 광고 3초 · ⏱ 1초 갱신 = `Overlay.OnTick` 신규) + 타임라인 제거·행 중앙 정렬(표 ㉒ 회차 정정 = 결정 155) + 로비 빨간 점 · **확인 = 그 커밋의 CI 유니티 잡(`UiSmokeTests` T77 구간 · `LayoutSpecTests` ㉒) + screens 17 PNG(§5 비평 ≥ 8.0) + 배포 스모크** · PROGRESS T77 진행 기록

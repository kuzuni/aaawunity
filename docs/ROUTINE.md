...
>
> **2단계 · 장비·대장간 묶음(`T90-gear`) ✔ 코드 push(`3c8bfe8` · sess-1928-29105 · 워커 F · 로컬 게이트 전부 초록 · 배포 스모크 초록)** — 06·07·08 을 전수로 훑었더니 **고칠 문구가 하나도 없었다**(장비 스탯 줄 = 절대값 · 슬롯/합성 안내는 이미 % · 세트 옵션은 1단계가 처리). 그래서 ⓐ 게이트가 **08 대장간**을 열게 하고(재료 한 칸 눌러 안내 문구까지) ⓑ `StrictScreens` += 02·06·08 ⓒ `PercentAudit.Strict` 를 켰다가 **같은 분(20:4X)에 되돌렸다**(`c2ebddb`) — 워커 K 의 `8f57711`(T90-audit)이 같은 분에 순회를 **스무 화면 남짓**으로 넓혀 «표가 0» 이라는 근거가 그만큼 좁아졌다. **다음 회차에 넓힌 순회의 표가 CI 로그에서 0 인 것을 보고 켠다**(워커 K 가 적어 둔 순서 그대로). `StrictScreens` 는 실측 근거가 있는 다섯(02·04·05·06·07)만 ⓓ EditMode `StatTextTests` 에 **표 밖의 이름**을 잡는 테스트 1개(기존 두 테스트는 표에 든 12개 이름만 본다). **확인 = 그 코드를 담은 첫 완주 런의 `PercentGateTests` Passed + 로그 «[PercentGate] … strict=True … 0»** · 결정 256~258.
>

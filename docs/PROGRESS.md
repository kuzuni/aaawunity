...
| T20 | 웨이브 출발 버그(연출) · 버프 스택 = 팔각 프레임 (구 T13) | ✅ 완료 (`81780ac` · 실물 확인 = CI 런 #42 PlayMode `BattleWorldTests` + 주인 에디터 플레이) | sess-0535-6643 / 워커 C | Game/BattleWorld(표시 원점 ShownPX · KillPending) · BattleScreen(RefreshBuffBar · World 접근자) · Tests/PlayMode/BattleWorldTests(신규 1) | **원인 = 연출**(엔진은 킬 다음 틱에 걷고 화면은 사망 연출을 칼 내려올 때까지 미룸) → 사망 연출 안 나온 적이 있으면 스크롤 원점 정지 · 풀리면 걷기 2배로 따라잡기 · 엔진 좌표 불변 · 버프 칸 = ItemFrame_04(PerkFrame) · dotnet 0/0 · 테스트 63/63 · 시뮬 21칸 동일 |
| T21 | 투사체 표적 = 웨이브 안 무작위(원본 sim.js 확인 후) — T20·T19 뒤 (구 T14) | 대기 | — | Core/Battle · Game/BattleWorld | ROUTINE §2 T14 |
| T22 | 모든 버튼 눌림 표시(ColorTint) (구 T15) | 대기 | — | Game/UiKit · Screens(NavBar) | ROUTINE §2 T15 |
| T23 | 쉼터 «광고 보고 둘 다» · 클리어 = 골드만 + «광고 보고 보상 ×2» — T21 뒤 (구 T16) | 대기 | — | Game/Overlay · BattleScreen · Core/Battle(ResolveRestBoth) | ROUTINE §2 T16 |
| T17 | 장비 아이콘 마무리 — 파츠 아이콘 크기 통일(칸 72%) · 무기 45° · 근접 무기(검·방망이)만 — T7 뒤 | 🔄 진행중 (코드 `6918f71` · dotnet 게이트 초록 · **CI 런 #41 유니티 잡 대기** — 이 세션이 깨어나 읽고 종결) | sess-0521-16751 / 워커 B | Core/GearLook(`FitPartIcon` 순수 계산 · `PartIconFill`·`WeaponIconAngle` · `WeaponSlot` 창 폐기) · Game/GearUi(`FitIcon`) · GearScreen(슬롯) · PartIconFit(신규) · catalog(회피 무기 4종 창→검) · Tests/EditMode/GearLookTests(+4) · Tests/PlayMode/UiSmokeTests(슬롯 아이콘 단언) | 파츠 아이콘 = 불투명 bbox(Tight 메시 정점) 긴 변이 칸의 72% · pivot = 그림 가운데 · 무기 +45°(칼끝 오른쪽 위) · GUI Pro 아이콘은 프리팹 값 복원 · 무기 12종 전부 Sword/Blunt 폴더 · dotnet 0/0 · 테스트 63/63 · 시뮬 21칸 동일 |
...
### T17 진행 기록 (2026-09-06 · sess-0521-16751 · 워커 B) — 코드 `6918f71` · CI #41 대기
...
- **플레이 콘솔 에러 0 확인 수단**: 코드 커밋 `6918f71` 의 **CI 런 #41**(https://github.com/kuzuni/aaawunity/actions/runs/34014374421) 유니티 잡 — PlayMode `UiSmokeTests` 5건(장비 화면 검사 지점마다 `PlayLog.AssertNoRed`) + 새 슬롯 아이콘 단언. 이 세션이 약 25분 뒤 깨어나 읽는다(빨가면 `result="Failed"` 메시지를 여기 붙이고 고친다).
...
- **플레이 콘솔 에러 0 확인 수단**: 코드 커밋 `81780ac` 의 **CI 런 #42**(https://github.com/kuzuni/aaawunity/actions/runs/34014908043) 유니티 잡 — PlayMode `BattleWorldTests` 1 + `UiSmokeTests.BattleTicksAndAllBattlePopups`(전투 화면 · 버프 바 포함). 이 세션이 약 30분 뒤 깨어나 읽는다(빨가면 `result="Failed"` 메시지를 여기 붙이고 고친다). 참고: 직전 런 #41(T17 · 워커 B)이 빨강이라 #42 도 같은 원인이면 T17 쪽 문제다.

...
이 꼬리에서 똑같이 보였다**. 건너뜀 자체는 허락된 꼴이라(「잴 것이 없는 판은 통과시킨다」 §1 ⓑ) **막지 않고 센다**:
    [CI명부] playmode-results.xml — 자 뭉치 95개 · 케이스 248건 · **건너뜀 2건** · 실패 0
    [CI명부] ⚠ **건너뜀 2건** — 돌았지만 «재지 않았다» …
    [CI명부]   1. ArenaResultTests.StageDiffers — 이 챕터의 무대가 마침 사막이라 …
⚑ **건너뜀 수는 0이어도 늘 찍는다 — 옆의 «실패 0» 과 같은 꼴로.** 안 찍으면 이 꼬리가 T503 이전과 **바이트까지 같아서**
...
def report_roster(path, echo=print):
    ...
    for fname, rows, total, bad, skip_rows, skipped in roster(path):
        ...
        #      «건너뛴 것이 없다» 와 **«세는 눈이 실물 XML 에서 안 먹는다»** 를 읽는 사람이 **못 가른다**.
        #      그러면 이 절이 없애려던 꼴(«건너뜀이 «실패 0» 에 숨는다»)이 **한 켜 위로 옮겨 앉는다** —
        #      이제 «안 먹는 눈» 이 «건너뜀 0» 에 숨는다(T278 그 자체다).
        #      결정 667 이 아끼라는 것은 **«줄»** 이고, 이것은 같은 줄에 글자 몇이 느는 것이다 —
        #      **아끼는 것은 «줄» 이지 «구별» 이 아니다.** 상세(이름·까닭)는 여전히 0건이면 한 줄도 안 찍는다.
        echo(f"{ROSTER_TAG} {fname} — 자 뭉치 {len(rows)}개 · 케이스 {total}건"
             + (f" · **건너뜀 {len(skipped)}건**" if skipped else " · 건너뜀 0")
...
def self_test():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        ok &= "케이스 3건 · **건너뜀 2건**" in joined        # 머리글에서 «실패» 와 같은 자리·같은 꼴로 선다
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        report_roster(d, lines.append)
        joined = "\n".join(lines)
        ok &= "케이스 2건 · 건너뜀 0 · **실패 1건**" in joined   # ⓐ 0 이 «실패» 와 같은 자리에 선다
        ok &= "돌았지만" not in joined                          # ⓑ 상세 블록은 0건이면 한 줄도 안 찍는다
        ok &= "뭉치:" not in joined

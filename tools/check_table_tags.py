...
⛔ **«아직 만들 수 없는 요소» 는 안 센다**(3회차) — 표의 **비고**에 `⛔` 가 있으면 그 행은 이름을 적기만 하고
   ...
   ⚠ 표시는 «비고» 에만 단다(행 이름·수치는 안 건드린다). T464 부터는 `ui_score` 채점도 같은 표시를 읽어 그 행을 **안 센다**
      (그 전에는 이 자만 빼고 채점은 «요소 없음 ✗0» 으로 세어 같은 표 같은 행을 두 자가 다르게 읽었다 · `19_pass` 10.0 → 8.2 가 그것).
...
NOT_YET = ui_score.NOT_YET      # 표의 «비고» 에 이 표가 있으면 «아직 만들 수 없는 요소» — 이름을 적되 어긋남으로 안 센다(3회차) · T464 부터 정의는 ui_score 한 곳

...
def main():
    ...
    if calling > 0 and judged > limit:
        ...
        if tip_calls is False:
            ...
            # ⛑ T500 — 처방을 따르면 **런이 빨갛게 끝난다**. 그 빨강이 남의 것인 줄 모르면 띄운 사람이
            #   «내가 무엇을 잘못 띄웠나» 로 한 회차를 쓴다(2026-09-12 런 1146 이 그 값을 치렀다).
            #   ⚠ 이 줄은 **그 사이 기간의 말**이다 — 뿌리는 T499(굽기 둘을 `inputs.build` 뒤로 · `ci.yml`)이고,
            #     그 손이 여기 와서 이 줄을 «입력 없이 띄우면 굽기는 건너뛴다» 로 바꾸면 두 자리가 같은 말을 한다.
            #     (검수 Q 가 T499 에 그렇게 적어 뒀다 — 자기가 만든 규약이 낳은 빨강이라 그 자리까지 세었다.)
            print("  ⤷ ⚠ **띄운 런은 빨갛게 끝난다 — 그 빨강은 «굽기» 둘이지 네 커밋이 아니다**: "
                  "`WebGL 빌드`·`Android APK 빌드` 는 `workflow_dispatch` 에서만 도는데 `UNITY_LICENSE` 가 없어 "
                  "«Missing Unity License File» 로 30초 만에 죽는다(T250 → T283 → T499). "
                  "**읽을 곳은 `unity-test` 잡과 `[CI실패]` 목록**이고, 거기가 0건이면 네가 갚으려던 빚은 갚아진 것이다. "
                  "(배포는 유니티 잡만 보므로 주인 폰은 안 다친다 · 뿌리 고침은 T499.)")

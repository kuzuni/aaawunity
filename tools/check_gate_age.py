...
def main():
    ...
    if calling > 0 and judged > limit:
        ...
        if tip_calls is False:
            ...
            # ⛑ T500 → T501 — **이 줄이 한 번 낡았다. 그 낡음이 이 자리의 교훈이다.**
            #   T500(워커 B)이 «띄운 런은 빨갛게 끝난다» 를 적었다 — 그때는 참이었다(런 1146).
            #   **한 시간 뒤 T499(워커 F)가 `ci.yml` 에서 그 뿌리를 고쳤고**(굽기 둘을 `inputs.build` 뒤로 · 기본 꺼짐),
            #   그 순간 이 줄은 **거짓**이 됐다 — 그런데 고친 손은 `ci.yml` 만 열었으므로 여기는 그대로 남았다.
            #   ⇒ B 의 주석이 «그 손이 여기 와서 바꾸면 두 자리가 같은 말을 한다» 로 예고한 그 자리를, **아무도 안 왔다**.
            #   ⚑ 그래서 이번에는 **어느 쪽으로 굴러도 참인 말**로 적는다 — 예언(«빨갛다»·«초록이다») 대신 **규칙**을 적는다.
            #     한쪽 결과를 박아 두면 뿌리가 움직일 때마다 이 줄이 다시 거짓이 되고, 거짓말하는 처방은 안 따르느니만 못하다.
            print("  ⤷ **굽기 둘은 같이 안 돈다** — `WebGL 빌드`·`Android APK 빌드` 는 `workflow_dispatch` 전용인데 "
                  "T499 뒤로 **`build` 입력을 켰을 때만** 선다(기본 꺼짐). 그냥 띄우면 그 둘은 `skipped` 다 — "
                  "이 처방이 부르는 것은 **유니티 자 한 번**이지 굽기가 아니다.")
            print("  ⤷ ⚠ **그래도 런이 빨갛거든 «굽기 둘인가» 부터 보라 — 그 빨강은 네 커밋이 아니다**: "
                  "그 잡들은 `unity-builder@v4` 라 **계정 방식을 안 받고** `UNITY_LICENSE` 가 없으면 "
                  "«Missing Unity License File» 로 30초 만에 죽는다(T250 → T283 → T298 → T499 · 실측 런 1146). "
                  "**읽을 곳은 `unity-test` 잡과 `[CI실패]` 목록**이고, 거기가 0건이면 네가 갚으려던 빚은 갚아진 것이다. "
                  "(배포는 유니티 잡만 보므로 주인 폰은 어느 쪽이든 안 다친다.)")

...
SCREENS = {
    ...
    # T332 — T299 가 «아무도 안 찍는 표» 로 부르던 셋. 레퍼런스 jpg 가 없는 화면이라 번호가 아니라 **이름**이다
    #        (`ev_*`·`res_*` 와 같은 규약) ⇒ 이름으로 표를 찾는 갈래가 «NN_» 접두를 못 찾으므로 **여기 적어야만** 채점된다.
    'lobby_menu': ('㉜', None, None),
    'shop_chest_open': ('㉞', None, None),
    # ㉟ 는 **한 표에 두 팝업**이라 행 앞머리로 갈린다(⑤·⑦ 과 같은 꼴).
    #   ⚠ 아바타 쪽을 «only 아바타» 가 아니라 «exclude 이름» 으로 둔 까닭: 그 표의 첫 행 «어둠(Dimmed)» 은 **두 팝업 공통**이라
    #     둘 다 only 로 두면 그 행이 어느 쪽에서도 안 세어진다(있는 것을 «없는 것» 으로 만드는 쪽으로 틀린다).
    'profile_avatar': ('㉟', None, '이름'),

...
- `profile_07.png` — space / `ui.face7`; corrupt source-branch blob rejected, then regenerated by the parent with built-in imagegen and the mandatory character/profile references
...
All 13 accepted files now match the mandatory `character_base.png` style. The replacement `profile_07.png` was inspected for identity, right-facing gaze, framing and genuine alpha, then registered under the unchanged `ui.face7` key.

## Remaining

- Parent integration: verify the unchanged nine-key profile order and ninth selection in Unity CI/screens.
...
- Parent revalidation: 13/13 accepted PNG files load fully and match the manifest SHA256. The corrupt source-branch `profile_07.png` was not used.
